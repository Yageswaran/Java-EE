...
public boolean register(Logon user){
...
public boolean isUserExists(Logon user){
	 Session session = HibernateUtil.getSessionFactory().openSession();
	 ...
	 try{
		 tx = session.getTransaction();
		 tx.begin();
		 Query query = session.createQuery("from Logon where username='"+user.getUsername()+"'");
		 Logon u = (Logon)query.uniqueResult();
		 tx.commit();
